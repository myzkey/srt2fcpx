...
import { parseSrt, formatSrtTimecode, stripHtmlTags } from './parser';
...
Text`;
      ...
      expect(result.cues).toHaveLength(0);
      expect(result.errors.some(e => e.includes('Invalid timecode format'))).toBe(true);
    });
...
Line 3`;
    ...
    it('should handle blocks with only one line (incomplete)', () => {
      ...
      expect(result.cues).toHaveLength(0);
      expect(result.errors.some(e => e.includes('Skipping incomplete block'))).toBe(true);
    });
...
Text`;
  ...
  describe('stripHtmlTags', () => {
    ...
    it('should remove tags with special characters in attributes', () => {
      expect(stripHtmlTags('<a href="http://example.com?foo=bar&baz=qux">Link</a>')).toBe('Link');
    });

    it('should remove content between < and > even if not valid tags', () => {
      expect(stripHtmlTags('Text with < and > characters')).toBe('Text with  characters');
    });
    ...
    it('should handle mixed content', () => {
      const input = 'Normal <b>bold</b> text with <i>italic</i> and <u>underline</u>';
